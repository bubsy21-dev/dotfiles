...
local done = hovercursor(wibox.widget {
	...
	buttons = {
		awful.button({}, 1, function()
			local w = assert(io.open(".config/awesome/sakura/json/user.json", "w"))
...
local function colorgen()
	...
	for dir in io.popen([[ls -d .config/awesome/sakura/color/*/ | cut -f4 -d'/']]):lines() do
...
local colorapply = hovercursor(wibox.widget {
	...
	buttons = {
		awful.button({}, 1, function()
			awful.spawn.easy_async_with_shell("~/.config/awesome/sakura/color/" .. colorentries[colorpos] .. "/" .. colorentries[colorpos] .. ".sh")
